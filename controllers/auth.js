...
router.post('/signup', async (req, res) => {
  ...
  const { email, firstName, initials, lastName, password, } = req.body; // goes and us access to whatever key/value inside of the object
  try {
    const [user, created] = await User.findOrCreate({
      where: { email },
      defaults: { firstName, lastName, initials, password }
    });
